...
class Person:
    ...
    def choose_accommodation(self):
        ...
        elif 3000 <= self.income <= 7000:
            self.accommodation = 'Standard Apartment'
        elif self.income < 3000 and self.social_status == 'Single':
            self.accommodation = random.choice(['Shared Housing', 'Public Housing'])
        elif self.social_status == 'Family' and self.income >= 3000:
            self.accommodation = 'House'
        elif self.income < 3000 and self.social_status == 'Family':
...
def create_persons_and_plot(num_persons, education_probs, employment_probs, income_min, income_max, social_status_probs, relatives_abroad_prob):
    ...
    for _ in range(num_persons):
        age = random.choices(['0-14', '15-64', '60-65',  '65+'], [0.316, 0.624, 0.104, 0.06])[0]
        gender = random.choices(['Female', 'Male'], [0.488, 0.512])[0]
...
def calculate_resource_usage(persons):
    accommodation_needs = {
        'Luxury Apartment': {'water': 200, 'electricity': 7.66, 'land': 65},
        'House': {'water': 200, 'electricity': 7.66, 'land': 55},
        'Standard Apartment': {'water': 200, 'electricity': 7.66, 'land': 46},
        'Shared Housing': {'water': 200, 'electricity': 7.66, 'land': 37},
        'Public Housing': {'water': 200, 'electricity': 7.66, 'land': 28},
...
# Streamlit UI
st.title('climate Migrants')
st.subheader('Tuvalu Migrants')

num_persons = st.number_input('Number of Persons', min_value=1, step=1, value=10000)
income_min = st.number_input('Minimum Salary ($)', min_value=0, value=3000)
income_max = st.number_input('Maximum Salary ($)', min_value=0, value=7000)

education_probs = [
    st.slider('No Education Probability', 0.0, 1.0, 0.05),
    st.slider('Primary Education Probability', 0.0, 1.0, 0.95),
    st.slider('Secondary Education Probability', 0.0, 1.0, 0.55),
    st.slider('Higher Education Probability', 0.0, 1.0, 0.60)
]
...
social_status_probs = [
    st.slider('Single Probability', 0.0, 1.0, 0.42),
    st.slider('Family Probability', 0.0, 1.0, 0.53)
]
